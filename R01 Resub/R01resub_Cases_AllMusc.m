%% R01 Resub - Subset Cases for IM comparison with Sleeve Data (ALL MUSCLES)
...
muscleName = chanelsLabel;

emgF = filtfilt(b,a,abs(data)')'; % Filter Data

mIdx = find(contains(chanelsLabel, muscleName)); % Index of Desired EMG Channels
emgF_task = emgF(mIdx,:); %Extract Relevant Channels
...
hSeries = figure; 
maximize;%
suptitle([char(taskName) ': 10 Cycles in Series']) %
% suptitle({'W - Pro/Sup - Neu - Flt:', '10 Cycles Overlapped'}); %

for idx=1:length(mIdx)
    hSE(idx) = subplot(length(mIdx),1,idx)
% %     figure; maximize;
    hold on;
...
%     set(hSE(idx), 'YtickLabel', 'fontsize', 6)
%     xlim([0 max(cycleLengths/Fs)]);%
    ylim([0 max(max(emgF_task))]);
    [xt] = xticks;
    set(hSE(idx),'Xticklabel',[]);
end
set(hSE(end),'Xticklabel',xt);
xlabel('Epoch Time (sec)');

saveas(hSeries,[char(taskPath) '\' char(taskName) '_InSeries_ALL.png']);
savefig(hSeries,[char(taskPath) '\' char(taskName) '_InSeries_ALL']);
...
for muscle2=1:length(mIdx)
    hSY(muscle2) = subplot(length(mIdx)/4,4,muscle2); %Toggle Comment for Subplot vs Figure.
%     hSY(muscle2) = figure; %maximize; %Toggle Comment for Subplot vs Figure.
    ...
    ylabel([char(muscleName(muscle2)) ' (log uV)']);
%     vline([cycleStarts/Fs],'g'); vline([cycleStops/Fs],'r:'); %
...
saveas(hSLogY,[char(taskPath) '\' char(taskName) '_LogScale_ALL.png']);
savefig(hSLogY,[char(taskPath) '\' char(taskName) '_LogScale_ALL']);
...
for idx=1:length(mIdx)
    hOL(idx) = subplot(4,length(mIdx)/4,idx);
% %     figure; maximize;
    hold on;
...
saveas(hOverlap,[char(taskPath) '\' char(taskName) '_Overlap_ALL.png']);
savefig(hOverlap,[char(taskPath) '\' char(taskName) '_Overlap_ALL']);
