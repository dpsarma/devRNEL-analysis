    ...
    for f = 1:length(freqs)
        ...
        for t = 1:length(eIdx)
            ...
            for i = 1:length(msubset)
                ...
                plot(trial(n).timeVec, trial(n).emg(m,:));                 
                hold on;
                vline(trial(n).stims/fs);
                title(Muscle_ID(m));ylabel('EMG (uV)'); xlabel('Time (s)');                 
            end
            tit = sgtitle(['E' num2str(trial(n).spinalElec) '- Full Trace: ' num2str(trial(n).stimFrequency) 'Hz_' ... 
...
% %             figure(h(ii,3)); subplot(4,2,c-8);
    ...
    %         zlabel('EMG (uV)');
        ...
        figure(h(p,f));
        tit = sgtitle(['E' num2str(trial(n).spinalElec) '- Waterfalls: ' num2str(check_f) 'Hz_' ... 
                 num2str(check_p) 'ms']);
        set(tit, 'Interpreter', 'none');
...
%% Plot Bars for Onsets
f = 4;
firstpeak = reshape(peakR(:,f,1:2:end),length(msubset),size(peakR,3)/2);
firstons = reshape(onset(:,f,1:2:end),length(msubset),size(peakR,3)/2);
secondpeak = reshape(peakR(:,f,2:2:end),length(msubset),size(peakR,3)/2);
secondons = reshape(onset(:,f,2:2:end),length(msubset),size(peakR,3)/2);

for i = 1:length(msubset)
    peak(i,1) = mean(peakR(i,f,1:2:end));
    peak(i,2) = mean(peakR(i,f,2:2:end));
    ons(i,1) = nanmean(onset(i,f,1:2:end));
    ons(i,2) = nanmean(onset(i,f,2:2:end));
    
    tmpOns = firstons(i,:);
    onserror(i,1) = nanstd( tmpOns ) / sqrt( length(tmpOns) );
    tmpOns = secondons(i,:);
    onserror(i,2) = nanstd( tmpOns ) / sqrt( length(tmpOns) );
    tmppks = firstpeak(i,:);
    pkserror(i,1) = std( tmppks ) / sqrt( length(tmppks) );
    tmppks = secondpeak(i,:);
    pkserror(i,2) = std( tmppks ) / sqrt( length(tmppks) );
...
tiledlayout(1,2);
nexttile
superbar(peak,'E', pkserror','BarFaceColor',permute(C, [3 1 2]));
ylabel('EMG (uV)'); ylim([0,inf]);
% tmp = [' ', mlab, ' '];
% xticklabels(tmp);
xticks([1:4]);
xticklabels(Muscle_ID(msubset));
legend('First Stim', 'Second Stim');
title({'Post-activation Depression:'; 'Peak Response'});
hold on;
XData = repmat([1:length(msubset)],6,1);


nexttile
superbar(ons,'E', onserror','BarFaceColor',permute(C, [3 1 2]));
ylabel('EMG (uV)');
% tmp = [' ', mlab, ' '];
% xticklabels(tmp);
xticks([1:4]);
xticklabels(Muscle_ID(msubset));
legend('First Stim', 'Second Stim');
title({'Post-activation Depression:'; 'Response Onset'});
hold on;
XData = repmat([1:length(msubset)],6,1);


saveas(hS6,['D:\FigRescources\UH3\LSP05\rehash\PAD\E' num2str(elecs) '_PAD_' num2str(check_f(f)) '_barplot.png']);
saveas(hS6,['D:\FigRescources\UH3\LSP05\rehash\PAD\E' num2str(elecs) '_PAD_' num2str(check_f(f)) '_barplot.svg']);
