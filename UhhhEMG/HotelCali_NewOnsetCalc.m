...
% %     [sortedStim,I] = sort([trial(eIdx).stimAmp]);
    ...
    for i = 1:length(msubset)%[2 10 3 11 4 12 5 13 9 16]%:length(msubset)
    m = msubset(i);
...
ax3 = nexttile;
superbar(mxonset_mean*1000','E', mxonserror*1000','BarFaceColor', C, 'Orientation', 'h');%permute(C, [3 1 2]));
xlabel('Response Max (ms)');
% tmp = [' ', mlab, ' '];
% xticklabels(tmp);
...
% legend(arrayfun(@num2str,elecs,'UniformOutput',false), 'Location', 'eastoutside', 'Orientation', 'vertical');
hold on;
sgtitle(['Subject 2, Response Latencies']);%subjectName
