    ...
    for mm = 1:length(Muscle_ID)
            ...
            for ss = 1:length(XS)
              ...
              if isempty(tmp) 
                  ...
                  RBinary(ii,mm) = sortedStim(tmp(1));
              end
...
spidermin(1:8) = 1000; spdrmax(1:8) = 6000; spdriter = 5; spdrfill = 'off';
for ii = 1:length(elecs)
   hS0 = figure; maximize;
    subplot(121); spider_plot([RBinary(ii,9:16)/1000;],...
        'AxesLabels', Muscle_ID(9:16),'AxesInterval', 2,...
        'FillOption', spdrfill,'FillTransparency', 0.1, 'AxesLimits',...
        [spidermin; spdrmax]/1000,...
    ...
    subplot(122); spider_plot([RBinary(ii,1:8)/1000;],...
    ...
    saveas(hS0,[reportPath '\' strrep(datestr(now),':','_')  '_ThresholdPlot' '_' num2str(elecs(ii)) '.png']);
    savefig(hS0,[reportPath '\' strrep(datestr(now),':','_')  '_ThresholdPlot' '_' num2str(elecs(ii))]);
    close all
...
hS1 = figure; maximize;
spdrfill = 'on';
    subplot(121); spider_plot([RBinary(:,9:16)/1000;],...
        'AxesLabels', Muscle_ID(9:16),'AxesInterval', 2,...
        'FillOption', spdrfill,'FillTransparency', 0.1, 'AxesLimits',...
        [spidermin; spdrmax]/1000,...
        'AxesInterval', spdriter, 'Direction', 'clockwise');
    ...
    subplot(122); spider_plot([RBinary(:,1:8)/1000;],...
        'AxesLabels', Muscle_ID(1:8),'AxesInterval', 2,...
        'FillOption', spdrfill,'FillTransparency', 0.1, 'AxesLimits',...
        [spidermin; spdrmax]/1000,...
        'AxesInterval', spdriter);
...
%     linkaxes([hF3s],'xy');
    ...
    if ~isempty(anodeElec)
        saveas(hF3,[reportPath '\' strrep(datestr(now),':','_')  '_bsrRecruitmentCurvesRMS' '_' setName '_ E' num2str(elecs(ii)) '-' num2str(anodeElec(eIdx(1))) '.png']);
        savefig(hF3,[reportPath '\' strrep(datestr(now),':','_') '_bsrRecruitmentCurvesRMS_' setName '_ E' num2str(elecs(ii)) '-' num2str(anodeElec(eIdx(1))) ]);
    else
        saveas(hF3,[reportPath '\' strrep(datestr(now),':','_')  '_bsrRecruitmentCurvesRMS' '_' setName '_ E' num2str(elecs(ii)) '.png']);
        savefig(hF3,[reportPath '\' strrep(datestr(now),':','_') '_bsrRecruitmentCurvesRMS_' setName '_ E' num2str(elecs(ii)) ]);
    end
end
close all
