...
for m = 1:length(Muscle_ID)
    ...
    for e = 1:length(peakInfo)
        ...
        for f = 1:length(peakInfo(e).muscle(m).standing)
            [mPk{e,f}, I_Pk] = max(peakInfo(e).muscle(m).standing(f).pks); %max peak value
            tPk{e,f} = peakInfo(e).muscle(m).standing(f).locs(I_Pk); %onset of max peak
            wPk{e,f} = peakInfo(e).muscle(m).standing(f).widths(I_Pk);
            width{e,f} = sum([peakInfo(e).muscle(m).standing(f).widths]);
        end
        stAmps{e,:} = [peakInfo(e).muscle(m).standing.stimAmp]; %stimAmp of eStim
        pkMns{e,:} = [peakInfo(e).muscle(m).standing.mean]; %mean/RMS activation
        pkVars{e,:} = [peakInfo(e).muscle(m).standing.var]; %var of peaks
        pkStd{e,:} = [peakInfo(e).muscle(m).standing.std]; %std of peaks
    end
% 
% for m = 1%:length(Muscle_ID)
%     for e = 1:length(peakInfo)
...
%         poly = polyfit(allStimAmps(idxValid),meanMaxPeaks(idxValid),3);
        ...
        hold off;
        tmptit = suptitle([Muscle_ID(m) 'Standing Stim, Response Summary']);
...
for i = 1:4
% %     linkaxes([hs(:,i)],'y'); %Comment out for Linked Plots
...
for  m = 1:length(Muscle_ID)
        ...
        saveas(hFig(m),[reportPath Muscle_ID{m} '_Standing_ResponseSummary_unlinked.png']);
        savefig(hFig(m),[reportPath Muscle_ID{m} '_Standing_ResponseSummary_unlinked']);
end
close all
